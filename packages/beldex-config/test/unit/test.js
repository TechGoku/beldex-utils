...
describe('Beldex Config', function () {
  describe('#config', function () {
    ...
    it('fetch open alias prefix from config', function () {
      assert.strictEqual("beldex", config.openAliasPrefix)
    })
    ...
    it('fetch dust threshold from config', function () {
      assert.deepStrictEqual(new JSBigInt('2000000'), config.dustThreshold)
    })
    ...
    it('fetch avg block time from config', function () {
      assert.strictEqual(120, config.avgBlockTime)
    })
  })
})
