...
const beldex_keyImage_cache_utils = require('@beldex/beldex-keyimage-cache')
...
function ownedParsedTxFrom__orNil(
  ...
  if ((tx.spent_outputs || []).length > 0) {
    for (var j = 0; j < tx.spent_outputs.length; ++j) {
      var key_image = beldex_keyImage_cache_utils.Lazy_KeyImage(
        keyImage_cache,
  ...
  tx.approx_float_amount = parseFloat(beldex_amount_format_utils.formatMoney(tx.amount))
...
function IsTransactionConfirmed(tx, blockchain_height) {
  ...
  return blockchain_height - tx.height > beldex_config.txMinConfirms
}
...
function IsTransactionUnlocked(tx, blockchain_height) {
  ...
  if (!beldex_config.maxBlockNumber) {
    throw 'Max block number is not set in config!'
  }
  if (unlock_time < beldex_config.maxBlockNumber) {
...
function TransactionLockedReason(tx, blockchain_height) {
  ...
  if (unlock_time < beldex_config.maxBlockNumber) {
    ...
    var unlock_prediction = moment().add(
      numBlocks * beldex_config.avgBlockTime,
      'seconds'
    )
    return (
      'Will be unlocked in ' +
