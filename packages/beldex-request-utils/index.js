...
function New_ParsedPayload_FromPossibleRequestURIString (string, nettype, beldex_utils/* pass this so this fn remains sync */) {
  // throws; -> {}
  //
  // detect no-scheme beldexAddr and possible OA addr - if has no beldex: prefix
  if (string.indexOf(coinUriPrefix) !== 0) {
    ...
    const couldBeOAAddress = string.indexOf('.') != -1 // contains period - would be nice to get this from DoesStringContainPeriodChar_excludingAsBDXAddress_qualifyingAsPossibleOAAddress so maybe beldex_core_js should gain local_modules/OpenAlias
    if (couldBeOAAddress) {
      return {
        address: string
      }
    ...
    try {
      address__decode_result = beldex_utils.decode_address(string, nettype)
    } catch (e) {
      throw Error('No Beldex request info')
    }
    // then it looks like a beldex address
    return {
      address: string
    }
  }
  ...
  if (protocol !== coinUriPrefix) {
    throw Error('Request URI has non-Beldex protocol')
  }
